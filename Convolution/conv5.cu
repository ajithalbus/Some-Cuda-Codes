...
for(int j=0;j<T;j++){
	for(int i=0;i<D;i++){
		slice[eY][i]=m[(eX+j-2)][eZ*ETL+eY][i];
...
__syncthreads();
	...
	if(!((eY<2)||(eY>TL-3))){
		...
		//if(eX==2&&eZ==0&&eY==TL-3) {printf("%f\n",psum);}
		atomicAdd(&o[filter][(eX-2)][eZ*(TL)+eY-2],psum);
	}
__syncthreads();
...
conv<<<dim3(N,W,2),12>>>(Dmatrix,Dtile,Doutput);
