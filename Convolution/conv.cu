...
void print_matrix_to_file1(int *m, unsigned numRows, unsigned numCols,unsigned d) {
	const char *fname = "mat";
	FILE *f = fopen(fname, "w");

	int (*mat)[numCols][d]=(int (*)[numCols][d])m;
...
__global__ void conv(int *matrix,float *tile,float *output){
...
int *matrix=(int*)malloc(sizeof(int)*W*W*D);
...
int *Dmatrix;cudaMalloc(&Dmatrix,sizeof(int)*W*W*D);
float *Dtile;cudaMalloc(&Dtile,sizeof(float)*N*T*T*D);
float *Doutput;cudaMalloc(&Doutput,sizeof(float)*(N*OW*OW));

cudaMemcpy(Dmatrix, matrix, sizeof(int)*W*W*D,cudaMemcpyHostToDevice);
